...
* **Organization API key:** View or copy the Organization API key. [Service accounts](broken-reference/) require an API key (token) to substitute for standard user credentials. Click the **Manage Service accounts** button to set up details for service accounts.
* **Organization ID**: View or copy the Organization ID, which is the internal ID for the Organization, generated automatically when each Organization is created. This ID uniquely identifies this Organization and is required when you use the [Snyk API](broken-reference). These Organization identification details are generated automatically by Snyk for each Organization when it is created.
* **Ignores**: Define Organization-wide ignore settings. You can choose from the following options:
  ...
  * **Require reason for each ignore**
    ...
    * `Not required`
    * See [Configure Ignore settings](broken-reference/) in the Ignore issues documentation for more details.
