...
* **A Snyk service account created specifically** to be used with the Kubernetes connector. For instructions on creating a service account, see [Service accounts](broken-reference). For the roles and permissions, Snyk recommends:
...
Next, create a new [service account](broken-reference) for Kubernetes connector integration with Snyk AppRisk.
