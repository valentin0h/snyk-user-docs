...
* If you do not have a `.snyk` file, you can create a file with a `.snyk` extension and paste the custom rule YAML formatted code into it for the rules to work. See [The .snyk file](broken-reference).

## Create a custom rule

You can save custom rules from useful queries and add metadata. The metadata is used to create YAML formatted code. Export the YAML content as a rule in the [.snyk file](broken-reference) of the relevant repository to run it alongside Snyk's in-house rules in future scans.
...
5. Configure the metadata:
   ...
   * **Severity**: Set the severity of the result based on [Snyk security levels](broken-reference). Snyk Code uses only three severity levels: `Low`, `Medium` and `High.`
...
7. Export the query as a rule by copying the YAML content to the [.snyk file](broken-reference) in the relevant repository.
